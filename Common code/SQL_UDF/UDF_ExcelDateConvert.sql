IF object_id('[dbo].[UDF_ExcelDateConvert]') is not NULL
BEGIN
   DROP FUNCTION [dbo].[UDF_ExcelDateConvert]
END
...
GO
CREATE FUNCTION UDF_ExcelDateConvert(@DateNumber AS VARCHAR(255))
RETURNS DATETIME
AS
/*
...
*/
BEGIN
	IF @DateNumber = '' OR CAST(@DateNumber AS FLOAT) = 0
		RETURN NULL
	...
	BEGIN
		RETURN CAST('Date outside acceptance range! See function UDF_ExcelDateConvert for details.' AS INT)
	END
